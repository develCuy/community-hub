---
title: Differences between Ethereum and Rollux
lang: en-US
---
...
However, on Rollux `tx.origin` is the origin *on Rollux*.
It could be an EOA.
...
If you need the equivalent information from the latest L1 block, you can get it from [the `L1Block` contract](https://github.com/SYS-Labs/rollux/blob/develop/packages/contracts-bedrock/contracts/L2/L1Block.sol).
This contract is a predeploy at address [`0x4200000000000000000000000000000000000015`](https://rollux.tanenbaum.io/address/0x4200000000000000000000000000000000000015).
...
| ---------------------------------- | ------------------------------------------ | 
| L2 user (Externally Owned Account) | The user's address (same as in Ethereum or Syscoin)   |
| L1 user (Externally Owned Account) | The user's address (same as in Ethereum or Syscoin)   |
...
1. Before Nimrod's swap transaction is sent to the blockchain, Helena Hacker sends a transaction from an L1 contract with the same address as Hackswap.
   ...
   Nimrod, despite his naivete, is protected because Rollux modifies the transaction's `tx.origin` (which is also the initial `msg.sender`).
...
The Rollux values are taken from [the Rollux specs](https://github.com/SYS-Labs/rollux/blob/develop/specs/guaranteed-gas-market.md#limiting-guaranteed-gas).
...
Rollux uses the same [JSON-RPC API](https://eth.wiki/json-rpc/API) as Ethereum.
...
This is a security risk. pre-EIP-155 transactions are not supported on Rollux.


## Transaction costs

[Transaction costs on Rollux](./transaction-fees.md) include an [L2 execution fee](./transaction-fees.md#the-l2-execution-fee) and an [L1 data fee](./transaction-fees.md#the-l1-data-fee). 


## Contract addresses

The addresses in which various infrastructure contracts are installed are different between L1 Syscoin and Rollux.
